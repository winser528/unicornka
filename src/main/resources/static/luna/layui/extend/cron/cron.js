layui.define(['jquery', 'form'], function (exports) {
  ...
  var $ = layui.$, form = layui.form,
      ...
      cron = {
        ...
        //主体CSS等待事件
        ready: function (fn) {
          layui.link(layui.cache.modules[MOD_NAME].replace('js', 'css?v=') +  cron.v, fn, "cron");
          return this;
        },
      },
      // 返回当前实例
  ...
  Class.prototype.lang = function () {
    var that = this,
        ...
        text = {
          ...
          en: {
            tabs: ["Seconds", "Minutes", "Hours", "Days", "Months", "Weeks", "Years",],
  ...
  Class.prototype.render = function () {
    ...
    elemMain.push(
        "</ul>",
        '<div class="layui-tab-content" style="width:600px;min-height:255px;">',
        elemSeconds,
        elemMinutes,
        elemHours,
        elemDays,
    ...
    // 底部区域
    var elemFooter = [
      '<div class="cron-box"><div class="cron-footer-btns">',
      '<span lay-type="confirm" class="cron-btns-confirm">确定</span></div></div>'
    ];
  ...
  Class.prototype.getSecondsElem = function () {
    var that = this,
        options = that.config,
        elem = [
          '<div class="layui-form layui-tab-item layui-show" lay-filter="cronSecForm">',
        ];
  ...
  Class.prototype.tool = function (btn, type) {
    var that = this,
        ...
        active = {
          ...
          calWeeks: function () {
            var data = form.val("cronWeekForm"), dataType = data["type[5]"];
            if (
                "range" == dataType &&
                isNotBlank(data.rangeStart) &&
                isNotBlank(data.rangeEnd)
  ...
  Class.prototype.position = function () {
    ...
    if (top + cronHeight + margin > winArea()) {
      top = rect.top > cronHeight //顶部是否有足够区域显示完全
          ? rect.top - cronHeight : winArea() - cronHeight;
      top = top - margin * 2;
    }

    that.elemCron.style.left = left + (options.position === "fixed" ? 0 : scrollArea(1)) + "px";
    that.elemCron.style.top = top + (options.position === "fixed" ? 0 : scrollArea()) + "px";
  };
  ...
  cron.render = function (options) {
    return thisIns.call(new Class(options));
  };

  exports(MOD_NAME, cron);
});
